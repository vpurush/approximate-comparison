...
class Hasher:
    ...
    def train(self, data, label):
        if data.shape[1] != self.dimension:
            raise ValueError("Data does not have proper shape" + str(data.shape) + " expected dimension is " + str(self.dimension))
    ...
    def isSimilar(self, data1, data2):
        return self.generateHashForData(data1) == self.generateHashForData(data2)
